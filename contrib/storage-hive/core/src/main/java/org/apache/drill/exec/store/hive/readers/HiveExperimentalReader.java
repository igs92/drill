...
/**
 * Reader which uses complex writer underneath to fill in value vectors
 * with data read from Hive. At first glance initialization code in the writer looks cumbersome,
 * but in the end it's main aim is to prepare list of key fields used in next() and readHiveRecordAndInsertIntoRecordBatch(Object rowValue) methods.
 * <p>
 * This class used in two stages: todo: mark description as simplified
 ...
 */
public class HiveExperimentalReader extends AbstractRecordReader {

  protected static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(HiveExperimentalReader.class);

  /**
   * Max amount of records that can be consumed by this mapredReader
   */
   ...
   * which encapsulate writing of column values read from Hive into
   ...
   * todo: try to delete the field after migration from partition vectors to partition vector writers
   */
  ...
  public void setup(OperatorContext context, OutputMutator output) throws ExecutionSetupException {
    ...
    Callable<Void> userSpecificInitTask = () -> {
      ...
      if (partition == null) {
        ...
        this.partitionToTableSchemaConverter = ObjectInspectorConverters.getConverter(partitionObjInspector, tableObjInspector);
      ...
      final List<String> partitionColumnNames = partitionKeyFields.stream().map(FieldSchema::getName).collect(Collectors.toList());
      ...
      ColumnProjectionUtils.appendReadColumns(job, idsOfProjectedColumns, selectedColumnNames,
          getColumns().stream().map(SchemaPath::getRootSegmentPath).collect(Collectors.toList()));
      ...
      */
      ...
      this.outputWriter = new VectorContainerWriter(output, /*enabled union*/ true );
    ...
    ListenableFuture<Void> initTaskFuture = context.runCallableAs(proxyUserGroupInfo, userSpecificInitTask);
    try {
      initTaskFuture.get();
  ...
  public int next() {
    ...
    try {
      ...
      // todo: may be necessary to rewrite to use common writer approach
