...
import org.apache.drill.exec.store.hive.readers.HiveExperimentalReader;
import org.apache.drill.exec.store.hive.readers.HiveExperimentalTextReader;
...
/**
 * The initializer is used as replacement of all other classes in this package.
 * In the POC all generated RecordReaders for Hive replaced by only two readers:
 * {@link HiveExperimentalReader} and {@link HiveExperimentalTextReader}.
 * This class instantiates them in {@link org.apache.drill.exec.store.hive.HiveScanBatchCreator}
 * without any reflections.
 */
public class ExperimentalReadersInitializer {

  private static final String TEXT_FORMAT = TextInputFormat.class.getCanonicalName();

  /**
   * Selects concrete experimental reader constructor reference as {@link HiveReaderFactory} readerFactory.
   ...
   * @return one or more experimental readers
   */
  public static List<RecordReader> init(ExecutorFragmentContext ctx, HiveSubScan config) {
    ...
    if (inputSplits.isEmpty()) {
      ...
      return IntStream.range(0, inputSplits.size()).mapToObj(idx ->
          readerFactory.createReader(config.getTable(), partitions.get(idx), inputSplits.get(idx), config.getColumns(), ctx, hiveConf, proxyUgi)
      ).collect(Collectors.toList());
    }
  }

  /**
   * Returns reference to {@link HiveExperimentalTextReader}'s constructor if
   * file being read has text format or reference to {@link HiveExperimentalReader}'s
  ...
  private static HiveReaderFactory getReaderFactory(HiveSubScan config) {
    ...
    return TEXT_FORMAT.equals(inputFormat) ? HiveExperimentalTextReader::new : HiveExperimentalReader::new;
  }
  ...
  /**
   * Functional interface used to describe parameters accepted by concrete experimental
   * readers constructor.
   */
  @FunctionalInterface
