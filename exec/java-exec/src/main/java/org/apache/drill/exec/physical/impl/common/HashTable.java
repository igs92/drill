...
public interface HashTable {
  ...
  /**
   * Enumeration of possible results for {@link HashTable#put(int, IndexPointer, int, int)} method.
   */
  enum PutStatus {
    KEY_PRESENT,
    KEY_ADDED,
   ...
   * @param cg
   */
  void setup(HashTableConfig htConfig,
  ...
  PutStatus put(int incomingRowIdx, IndexPointer htIdxHolder, int hashCode, int batchSize)
   ...
   * @return Returns -1 if the data in the probe batch at the given incomingRowIdx is not in the hash table.
   * Otherwise returns the composite index of the key in the hash table (index of BatchHolder and record in Batch Holder).
   * @throws SchemaChangeException
   */
  ...
  /**
   * Update the initial capacity for the hash table. This method will be removed after the key
   * vectors are removed from the hash table. It is used to allocate {@link HashTableTemplate.BatchHolder}s
   * of appropriate size when the final size of the HashTable is known.
   *
  ...
  /**
   * Changes the incoming probe and build side batches, and then updates all the value vector references in
  ...
  /**
   * Retrieves the key columns and transfers them to the output container. Note this operation removes the key
   * columns from the {@link HashTable}.
